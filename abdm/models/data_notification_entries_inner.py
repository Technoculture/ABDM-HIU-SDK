...
class DataNotificationEntriesInner(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, EntryContent):
            error_messages.append(f"Error! Input type `{type(v)}` is not `EntryContent`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in DataNotificationEntriesInner with anyOf schemas: EntryContent, EntryLink. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into DataNotificationEntriesInner with anyOf schemas: EntryContent, EntryLink. Details: " + ", ".join(error_messages))
        else:
            return instance
