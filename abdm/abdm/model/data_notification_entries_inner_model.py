...
DATANOTIFICATIONENTRIESINNERMODEL_ANY_OF_SCHEMAS = ["EntryContentModel", "EntryLinkModel"]
...
class DataNotificationEntriesInnerModel(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, EntryContentModelModel):
            error_messages.append(f"Error! Input type `{type(v)}` is not `EntryContentModelModel`")
        else:
            return v

        # validate data type: EntryLinkModelModel
        if not isinstance(v, EntryLinkModelModel):
            error_messages.append(f"Error! Input type `{type(v)}` is not `EntryLinkModelModel`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in DataNotificationEntriesInnerModel with anyOf schemas: EntryContentModel, EntryLinkModel. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into DataNotificationEntriesInnerModel with anyOf schemas: EntryContentModel, EntryLinkModel. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], EntryContentModel, EntryLinkModel]]:
