...
class DataNotificationModel(BaseModel):
    ...
    page_number: StrictInt = Field(
        description="Current page number.", alias="pageNumber"
    )
    page_count: StrictInt = Field(
        description="Total number of pages.", alias="pageCount"
    )
    transaction_id: StrictStr = Field(
        description="Transaction Id issued when data requested.", alias="transactionId"
    )
        ...
        _obj = cls.model_validate(
            {
                ...
                "entries": [
                    DataNotificationEntriesInnerModelModel.from_dict(_item)
                    for _item in obj["entries"]
                ]
                if obj.get("entries") is not None
                else None,
                "keyMaterial": KeyMaterialModelModel.from_dict(obj["keyMaterial"])
                if obj.get("keyMaterial") is not None
                else None,
            }
        )
        return _obj
